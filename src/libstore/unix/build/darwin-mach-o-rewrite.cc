...
bool fixupSlice(std::string & data, size_t sliceBase, const std::filesystem::path & path)
{
    ...
    /* Walk every blob in the SuperBlob and recompute hashes for each
       CodeDirectory we find. There may be more than one (binaries that
       carry both a SHA-1 and a SHA-256 CodeDirectory for compatibility
       with older macOS); we only touch SHA-256 ones. */
...
size_t fixupFile(const std::filesystem::path & path)
{
    ...
    if (magicLE == MH_MAGIC || magicLE == MH_MAGIC_64) {
        ...
        /* 32-bit fat: 8-byte header (FAT_MAGIC + nfat_arch) followed
           by an array of `fat_arch` entries (cputype, cpusubtype,
           offset, size, align — all big-endian uint32_t, 20 bytes).
           Apple's `fat_arch` struct is byte-swapped on little-endian
           hosts so we read fields by hand. */
