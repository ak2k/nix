...
#
# Trigger condition: the bug only fires when at least one sibling output
# is already in the store at the start of a multi-output build, so that
...
#
# Assertions on the `.check` binary:
...
if ! command -v codesign >/dev/null 2>&1; then
    skipTest "Need codesign(1) to validate signatures"
fi
...
if [[ "${NIX_TESTS_CA_BY_DEFAULT:-}" == "1" ]]; then
    ...
    # post-rewrite-and-fixup binary.
    expect 1 nix-build --no-out-link --check --keep-failed ./macho-rewrite.nix
    target="${out}.check"
    [[ -x "$target/bin/hello" ]]
fi
...
# pre-rewrite bytes)
echo "$output" | grep -qE '^doc=/nix/store/[a-z0-9]{32}-macho-rewrite-multi-doc/share/doc/hello$'

# Step 3 — codesign verifies the (rewritten + fixed-up) signature
codesign --verify "$target/bin/hello"
...
codesign -dvvv "$target/bin/hello" 2>&1 | grep -qF 'flags=0x20002(adhoc,linker-signed)'
