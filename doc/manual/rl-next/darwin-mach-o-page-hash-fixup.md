...
issues: [6065]
# prs: [NNNNN]  # fill at posting time
---

On darwin, the build daemon's placeholder-substitution pass
(`RewritingSink`) mutated bytes inside pages that `ld -adhoc_codesign`
had already covered with page hashes in `LC_CODE_SIGNATURE`. Affected
multi-output derivations produced binaries whose stored page hashes no
longer matched the on-disk bytes, and the macOS kernel SIGKILLed them
at first page-in with `cs_invalid_page`. This surfaced in nixpkgs as
[#507531](https://github.com/NixOS/nixpkgs/issues/507531) (fish on
`nixpkgs-darwin`).
